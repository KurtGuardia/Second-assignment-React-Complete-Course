...
import "./App.css";
import Validator from "./Components/ValidatorComponent";
...
class App extends Component {
  ...
  addChar = (e) => {
    let textToSplit = e.target.value;
    let text = textToSplit.split("");
  ...
  render() {
    let char = (
      <div>
        {this.state.char.map((ch, index) => {
          return <Char value={ch} key={Math.random()} click={() => this.delete(index)} />;
        })}
      </div>
    );
    ...
    return (
      <div className="App">
        ...
        <input value={this.state.char.join('')} onChange={this.addChar} /*onChange={this.addLength}*/></input>
        <p>The length: {this.state.length}</p>
        <Validator lenght={this.state.length} />
        {char}
      </div>
    );
  }
